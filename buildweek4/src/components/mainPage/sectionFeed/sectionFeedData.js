export const sectionFeedData = [
  {
    authorSrcImg: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTelpKg4p4D-ND351qo9IkaeiGwQMtFcyOQ6Q&s",
    authorName: "KMPG US",
    authorFollowersNum: "1.943.46",
    caption: "Discover how software industry leaders are reinventing their growth strategies, refining cost structures, and leveraging M&A amid the AI revolution. Our latest KPMG report offers insights you don’t want to miss: https://ow.ly/Ex3350XGj0C",
    postSrcImg: "https://media.licdn.com/dms/image/v2/D4E10AQHapRE-uaTLSg/image-shrink_1280/B4EZsS3eM4GYAM-/0/1765548090691?e=1770559200&v=beta&t=ifh_SsmTbt0ztLDk4Sm1zt5ZOCZfkqKcY6VjOjll_5Q",
    timeAgo: "3 giorni",
    isSponsored: false,
  },
  {
    authorSrcImg: "https://www.corporatephotographylondon.com/wp-content/uploads/2018/08/1-copy-3.jpg",
    authorName: "Cristiana Ferrari",
    authorJobTitle: "Recruitment Specialist - Politiche Attive presso Gi Group",
    caption: `Sono felice di condividere che ho completato questo percorso formativo: Introduzione all’Orientamento Formativo e Professionale presso ASNOR Associazione Nazionale Orientatori !`,
    postSrcImg: "https://media.licdn.com/media/AAYABAQSAAgAAQAAAAAAASTRcCTKRvmFS16-BzEr-Ye2FQ.gif",
    timeAgo: "1s",
    isSponsored: false,
  },
  {
    authorSrcImg: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoQa8eKoIuUxSOk5K6vnS0xL4KGnZByqnEQg&s",
    authorName: "Capgemini",
    authorFollowersNum: "8.537.831",
    caption: `It would be passé to ask if you use Gen AI, so how much time do you spend on Gen AI tools per day? 
    
    Our latest study shows that AI has become the new consumer companion. 
    AI tools now rank third in the top consumer subscription categories, behind movie and music subscriptions. Interactions with AI tools (multiple times a day) have nearly doubled since 2023.

    Certain trends in age demographics and use cases have been observed in the usage of AI tools as well: 
    ➡️ Over 50% of consumers view AI as a source of information and a collaborator. 
    ➡️ Older generations, especially boomers, are seeking emotional support via AI platforms. 
    ➡️ 7 in 10 Gen Z consumers are actively exploring AI tools. 
    ➡️ 50% of consumers are comfortable using an AI shopping assistant across platforms.

    Looking to better align your organization’s AI innovations with consumer expectations? 

    Download our research brief “From hype to habit” for critical insights: https://bit.ly/3MT0id2`,
    postSrcImg: "https://media.licdn.com/dms/image/v2/D5622AQGa1zcIVrbp6g/feedshare-shrink_2048_1536/B56ZsNd7i2HAA0-/0/1765457510155?e=1771459200&v=beta&t=_5vLN19nDFQE3NEkk4vgGhdnHlFGoth9o6wdowlbRBo",
    timeAgo: "4 giorni",
    isSponsored: false,
  },
  {
    authorSrcImg: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSt4Oc4WBgxZ0KbEjfm1W067bK4yUEAntn3bw&s",
    authorName: "Accenture",
    authorFollowersNum: "14.201.906",
    caption: `From “always on Zoom” to “always outdoors”, we’ve got gifting covered. Our guide is tailored to real people — the adventurer, the techie, the homebody — so you can give thoughtfully and with purpose. Discover global gifting ideas at the Accenture Store. https://accntu.re/4ogJc5R

    [Image Description: A slideshow of six gifting personas from the Accenture Gift Guide — the Well-being Warrior, Tech Trailblazer, Ultimate Sports Fan, Cozy Homebody, Weekend Wanderer, and Accenture Enthusiast — each surrounded by themed gift items, from yoga mats and candles to drones, jerseys, blankets, and branded Accenture gear.]`,
    postSrcImg: "https://media.licdn.com/dms/image/v2/D5610AQGqpM6HU7TTcw/ads-document-images_1280/B56ZrFZjSOHkAo-/1/1764248405340?e=1770854400&v=beta&t=A6OyE8_pNRnNbisbmq_KAQodT9unBNzdEzspRgIuQlI",
    timeAgo: "2s",
    isSponsored: false,
  },
]